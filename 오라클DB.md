...
특정 열의 데이터가 NULL일 경우에 연산 수행을 위해 데이터를 NULL이 아닌 다른 값으로 대체해 주어야 할 때가 종종 발생한다. 이때 NVL 함수와 NVL2 함수를 유용하게 사용할 수 있다.
...
![image-20250327215649711](C:\Users\bb\Desktop\OracleDB\assets\image-20250327215649711.png)

EMP 테이블의 급여 외 추가 수당을 의미하는 COMM 열 값이 NULL인 데이터를 0으로 대체하여 연산 가능하다.

### NVL2 함수의 기본 사용법

NVL2 함수는 NVL 함수와 비슷하지만 데이터가 NULL이 아닐 때 반환할 데이터를 추가로 지정해 줄 수 있다. NVL2 함수의 기본 형식은 다음과 같다.
...
예를 들어 COMM 열이 NULL이 아니라면 O를, NULL이라면 X를 표기하여 급여 외 추가 수당의 존재 여부만 확인할 때 NVL2 함수를 사용할 수 있다. 왜냐면 금액 같은 민감한 데이터는 노출되지 않아야 하는 경우도 있기 때문이다. 
...
![image-20250327220831328](C:\Users\bb\Desktop\OracleDB\assets\image-20250327220831328.png)

NVL2 함수는 NVL 함수와는 달리 NULL이 아닌 경우에 반환 데이터까지 지정할 수 있으므로 좀 더 다양한 용도로 활용 가능하다.

## DECODE 함수와 CASE문

NVL, NVL2 함수는 데이터가 NULL인 경우에 어떤 데이터를 반환할지 정하는 함수이다. 특정 열 값이나 데이터 값에 따라 어떤 데이터를 반환할지 정할 때는 DECODE 함수 또는 CASE문을 사용한다.
...
![image-20250327223509580](C:\Users\bb\Desktop\OracleDB\assets\image-20250327223509580.png)

DECODE 함수는 한 행으로 입력받아 한 행으로 결과가 나오는 단일행 함수이다. 만약 DECODE 함수의 맨 마지막 데이터, 즉 조건에 해당하는 값이 없을 때 반환 값을 지정하지 않으면 NULL이 반환된다.
